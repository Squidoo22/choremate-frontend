const REPLIES = {
  uk: {
    ...
    credit:
      "Ідея Кредиту Довіри: замість докору — приємний жест. Наприклад, «готує вечерю при свічках» або «робить каву в ліжко». Випишіть його у вкладці «Кредит Довіри» — це весело й без образ.",
    talk:
    ...
    default:
      "Я допоможу з побутом без сварок: справедливо розподілити справи, підказати ідею Кредиту Довіри чи як спокійно про це поговорити. Що саме турбує?",
  },
  en: {
    split:
    ...
    credit:
      "Trust Credit idea: instead of blame, a nice gesture — e.g. “cooks a candlelit dinner” or “brings coffee to bed”. Issue it in the “Trust Credit” tab — playful and blame-free.",
    talk:
    ...
    default:
      "I help keep the household calm: split chores fairly, suggest a Trust Credit idea, or how to talk about it without a fight. What's on your mind?",
  },
};
...
function classify(text) {
  ...
  if (/кредит|довір|жест|подарунок|загладити|credit|trust|gesture|make up/.test(t)) return "credit";
  ...
  if (intent === "credit") {
    if (ctx.debts && ctx.debts.length) {
      ...
      return en
        ...
        : `Зараз активний кредит: ${d.debtor} винен ${d.creditor} — «${d.description}».`;
    }
    return en
      ? "There are no active credits right now — you can issue one next time a task is missed."
      : "Активних кредитів зараз немає — можете виписати новий, коли хтось прострочить задачу.";
  }
